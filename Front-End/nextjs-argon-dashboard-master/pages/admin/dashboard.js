...
const Dashboard = (props) => {
  ...
  useEffect(async ()=>{
      ...
      const allNFts = await fetch('/api/getAllNFTS')
      const response = await allNFts.json();
      console.log(response);

      let myNftImages = JSON.parse(response.data)

      let nftData = [];
      for (let i = 0; i< myNftImages.length; i++){
        nftData.push({id: myNftImages[i]._id ,image: myNftImages[i].URL, estate: myNftImages[i].State});
      }
      ...
      <Container className="mt--7" fluid>
        ...
        <Row className="mt-5">
          ...
          {nftImages.map((nft, index) => {
            console.log(nft)
            return(
            <Col sm="6" lg="6" xl="3" key={index+1}>
            <Blog
              image={nft.image}
              estate={nft.estate}
              id = {nft.id}
