...
function Profile() {
  ...
  useEffect(async () => {
    ...
    const getBal = await fetch(`https://stacks-node-api.testnet.stacks.co/extended/v1/address/${myStxAddress()}/stx`)

    const userBal = await getBal.json();

    

    const response = await fetch(`/api/getUserNFTS?StxAddress=${myStxAddress()}`);

    const result = await response.json();

    console.log(JSON.parse(result.data));

    setBalance(userBal.balance/1000000);
